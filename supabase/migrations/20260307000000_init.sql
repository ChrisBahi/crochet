...
-- ── ROOM VALIDATIONS (confirmation bilatérale du deal) ───────────
create table if not exists room_validations (
  id          uuid primary key default uuid_generate_v4(),
  room_id     uuid references rooms(id) on delete cascade,
  user_id     uuid references auth.users(id) on delete cascade,
  created_at  timestamptz default now(),
...
create policy "room_validations_member" on room_validations
  for all using (
    ...
    exists (
      select 1 from rooms r
      join workspace_members wm on wm.workspace_id = r.workspace_id
      where r.id = room_validations.room_id and wm.user_id = auth.uid()
    )
  );

create index if not exists idx_room_validations_room on room_validations(room_id);
...
--   ADMIN_EMAILS=votre@email.com,autre@email.com
--   NEXT_PUBLIC_SITE_URL=https://votre-domaine.com

create index if not exists idx_admission_requests_status on admission_requests(status, created_at desc);
